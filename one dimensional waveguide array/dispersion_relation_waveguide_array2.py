...
Z=20e-2             #distance to travel in Z axis
...
for i in range (1, 7):
    init_field=init_field+gaussian_function(x_points-i*d, w0)*np.random.random()**np.exp(j*2*np.pi*np.random.random())+gaussian_function(x_points+i*d, w0)*np.random.random()*np.exp(j*2*np.pi*np.random.random())
    temp=init_field
'''   
for i in range (1, 10):
    a=np.random.random()
    b=np.random.random()
    init_field=init_field+init*a*np.exp(j*np.pi*b)
    temp=init_field
'''
#Main Code
